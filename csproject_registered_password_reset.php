...
/*
$button5 = isset($_POST['resetPass']);
	
if($button5){
	
	$pass = stripslashes(htmlentities(md5($_POST['newPass'])));
	
	$checkpass = mysql_query("SELECT * FROM info WHERE password = '$pass'");

	$num = mysql_num_rows($checkpass); // check the numbers of row where condition is true.

	if ($num > 0) {
		
		$res = mysql_fetch_assoc($checkpass);  // associative array.
		$dbusername = $res['username'];	// stored in the password column in the database.
								...
								<!-- Main Content -->
									<section>
										...
										<p> <font size="5px" face="Calibri">  
											...
											Please fill this form to reset your password. <br><br>
										
											<form action = "csproject_registered_password_reset.php?update=password" method = "POST">
												Enter your username <input type = "text" name = "user"><br><br>
												Enter a new password <input type = "password" name = "newPass">
											...
											<?php
												if(isset($_GET['update']) && $_GET['update']== "password"){
													...
													if (resetPassword($user, $newpass)) {
														echo '<font style = "color: yellow">'."Password updated! Please login with your new password.".'</font>';
											...
											<li style = "float: right; text-align: left; margin-right: 160px; margin-top: 80px; width: 100%; display: inline; width: 500px; ">
											...
											<form action = "csproject_logged.php" method = "POST">
												...
												<br>
												<br>
												Username: <input type = "text" name = "username" value = "<?php echo $dbusername;?>"<br><br>
